    ...
    public function run(): void
    {
        ...
        $dados = [
            ...
            [
                ...
                'province'           => 'Luanda',
                'participation_mode' => 'presencial',
                'observations'       => 'Interesse especial em psiquiatria infanto-juvenil.',
                'status'             => 'pendente',
                'curso_ordem'        => 3,
            ],
            [
        ...
        foreach ($dados as $dado) {
            $curso = $cursos->firstWhere('ordem', $dado['curso_ordem']) ?? $cursos->first();
            ...
            $inscricao = Inscricao::create([
                ...
                'status'             => $dado['status'],
                // sem user_id — participantes não têm conta
                'avaliado_por'    => in_array($dado['status'], ['aprovada', 'rejeitada', 'em_analise']) ? $adminId : null,
                'avaliado_em'     => in_array($dado['status'], ['aprovada', 'rejeitada']) ? now()->subHours(rand(1, 48)) : null,
        ...
        $this->command->info('✅ ' . count($dados) . ' inscrições de teste criadas (sem contas de utilizador).');
    }
}